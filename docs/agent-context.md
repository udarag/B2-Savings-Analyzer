...
- Optional commit-upsell screenshot parsing: `ANTHROPIC_API_KEY` (Claude vision reads uploaded B2 usage screenshots). Absent, that upload degrades to manual entry — no other feature depends on it.
...
- The commit-upsell form's screenshot upload reads a customer's B2 usage summary via Claude vision. `POST /api/analyses/[id]/usage-screenshot` always stores the image, then — when `ANTHROPIC_API_KEY` is set — calls `parseUsageScreenshot()` in `src/lib/analysis/usage-screenshot-parse.ts` (server-only; `claude-opus-4-8`) to read a few table cells and derive the form fields in code (GB→TB, spend passthrough, growth annualized from the observed trend and clamped). It returns `{status: 'parsed', parsed}` on success and the client pre-fills the form; with no key (`'unavailable'`) or a parse failure (`'failed'`) the AE falls back to manual entry. Parsing is best-effort and never blocks. This is the app's **first external-LLM dependency** (`@anthropic-ai/sdk` + `ANTHROPIC_API_KEY`); the SDK is imported server-side only (`import type` on the client) so it stays out of the client bundle.
