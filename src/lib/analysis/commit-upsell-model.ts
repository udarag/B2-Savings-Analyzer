...
// Compute path for the commit-upsell flow: an existing B2 Uncommitted customer being pitched to
// sign a contract (Committed) or jump straight to Overdrive. There's no source-cloud bill here —
// the only "current" numbers are what the AE entered about the customer's own B2 usage — so this
// stays a small, separate function rather than being shoehorned through ParsedBill/computeCostModel.
...
export function computeCommitUpsellView(usage: B2UsageInput): CommitUpsellView {
  ...
  // The one real fact we have is the customer's current implied $/TB; Committed is modeled as that
  // rate minus any AE-negotiated discount (0 by default — flat $/TB is the common case), Overdrive
  // substitutes its own suggested starting rate since it's a different, separately-negotiated tier.
  const impliedRatePerTb = usage.currentStorageTb > 0
    ? usage.currentMonthlySpendUsd / usage.currentStorageTb
    : 0;
  const targetRatePerTb = usage.targetTier === 'overdrive'
    ? targetSpec.startingPerTbMonth ?? impliedRatePerTb
    : impliedRatePerTb * (1 - usage.committedDiscountPercent / 100);
